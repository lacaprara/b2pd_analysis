...
void B0_ch5::Loop(Long64_t maxEv)
{
...
void B0_ch5::createHisto(const TString& dir) {
  ofile->cd();
  ofile->mkdir(dir);
  ofile->cd(dir);
...
void B0_ch5::saveHisto(const TString& dir) {
  ofile->cd();
  ofile->cd(dir);
...
void B0_ch5::fillHistos(const TString& dir) {
  ofile->cd(dir);
  ...
  ((TH1F*)gDirectory->Get("hMinvPi0"))->Fill(B0_etaP_eta_pi0_M);
...
void B0_ch5::fillHisto(const TString& dir, const TString& hname, const double& value) {
  TH1F* htmp=(TH1F*)ofile->Get(dir+"/"+hname);
  if (htmp) htmp->Fill(value);
}

Bool_t B0_ch5::Notify()
{
...
Int_t B0_ch5::Cut(Long64_t entry)
{
  ...
  if (B0_etaP_eta_M<0.52 || B0_etaP_eta_M>0.57) return 0;
  if (B0_etaP_M<0.93 || B0_etaP_M>0.98) return 0;
  if (B0_etaP_eta_pi0_M<0.1 || B0_etaP_eta_pi0_M>0.150) return 0;
  if (B0_K_S0_M<0.4 || B0_K_S0_M>0.52) return 0;
...
Long64_t B0_ch5::selectBestCand(Long64_t jentry, Long64_t nCandsCurrent) {
