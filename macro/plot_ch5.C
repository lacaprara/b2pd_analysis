...
void channel(TCanvas* c, float x=0.5, float y=0.96) {
  ...
  tch->SetTextAlign(21);
  TString channel("B^{0}#rightarrow#eta'( #eta(#pi^{+}#pi^{-}#pi^{0}) #pi^{+}#pi^{-}) K^{0}_{S}(#pi^{0}#pi^{0})");
  c->cd();
...
void plot_ch5() {
  TFile* hfile=TFile::Open("Histo_ch5.root");
  ...
  if (false) {
    ...
    float hCutLow[13] ={5.25,-0.15,0.52,0.93,0.1 ,0.40,0.2,-0.08,-0.15,1,1E-3,1E-3,1E-3};
  ...
  if(true) {
    ...
    float hCutLow[6] ={5.25,-0.15,0.52,0.93,0.1 ,0.40};
    ...
    c1->Print("Ch5_distr.pdf");
    c1->Print("Ch5_distr.png");

    TCanvas* c2=new TCanvas("c2","Before Cuts",900,600);
    c2->Divide(3,2);
    TLegend* tleg=new TLegend(0.2,0.6,0.8,0.89);
    tleg->SetFillStyle(0);
    for (int i=0; i<6; ++i) {
      c2->cd(i+1);
      hfile->cd("BestCandidates");
    ...
    c2->Print("Ch5_best_distr.pdf");
    c2->Print("Ch5_best_distr.png");
  }
  ...
  if (true) {
    ...
    cEvents->Print("Ch5_events.pdf");
    cEvents->Print("Ch5_events.png");
  }
  ...
  cDt->Print("Ch5_Dt.pdf");
  cDt->Print("Ch5_Dt.png");
  ...
  cAsym->Print("Ch5_Asym.pdf");
  cAsym->Print("Ch5_Asym.png");
}
