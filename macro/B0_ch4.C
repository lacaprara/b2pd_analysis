...
void B0_ch4::createHisto(const TString& dir) {
  ofile->cd();
  ofile->mkdir(dir);
  ofile->cd(dir);
...
void B0_ch4::saveHisto(const TString& dir) {
  ofile->cd();
  ofile->cd(dir);
...
void B0_ch4::fillHistos(const TString& dir) {
  ofile->cd(dir);
...
void B0_ch4::fillHisto(const TString& dir, const TString& hname, const double& value) {
  TH1F* htmp=(TH1F*)ofile->Get(dir+"/"+hname);
  if (htmp) htmp->Fill(value);
}
Bool_t B0_ch4::Notify()
{
...
Int_t B0_ch4::Cut(Long64_t entry)
{
  // This function may be called from Loop.
  // returns  1 if entry is accepted.
  // returns -1 otherwise.
  if (B0_mbc<5.25) return 0;
  if (B0_deltae<-0.15 || B0_deltae>0.15) return 0;
...
Long64_t B0_ch4::selectBestCand(Long64_t jentry, Long64_t nCandsCurrent) {
