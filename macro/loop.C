...
void plot() {
  TFile* hfile=TFile::Open("Histo.root");
  ...
  c1->Divide(3,2);
  TLegend* tleg=new TLegend(0.2,0.4,0.6,0.9);
  for (int i=0; i<6; ++i) {
    c1->cd(i+1);
    hfile->cd("AllCandidates");
    TH1* htmp=(TH1F*)gDirectory->Get(hNames[i]);
    ...
    if (htmp_is) {
      htmp_is->SetLineColor(kRed);
      htmp_is->DrawCopy("same");
      if (i==0)tleg->AddEntry(htmp_is,"Best cands","l");
    }
    hfile->cd("BestCandidatesIsSignal");
    TH1* htmp_is=(TH1F*)gDirectory->Get(hNames[i]+"_is_best");
    if (htmp_is) {
      htmp_is->SetLineColor(kViolet);
      htmp_is->DrawCopy("same");
      if (i==0)tleg->AddEntry(htmp_is," \" MC match","l");
    }
    tl->DrawLine(hCutLow[i],ymin,hCutLow[i],htmp->GetMaximum()*1.05);
    tl->DrawLine(hCutHigh[i],ymin,hCutHigh[i],htmp->GetMaximum()*1.05);
    if (i==0) tleg->Draw();
  ...
  TLegend* tleg=new TLegend(0.2,.7,0.4,0.9,"Fit","NDC");
  ...
  TLegend* tleg=new TLegend(0.2,.7,0.4,0.9,"Flavour","NDC");
