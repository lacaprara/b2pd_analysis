...
class B0_ch2 {
  public :
    ...
    B0_ch2(TTree *tree=0,const char* outAppendix="");
    bool _skipIfSignal;
    virtual ~B0_ch2();
...
#ifdef B0_ch2_cxx
B0_ch2::B0_ch2(TTree *tree, const char* outAppendix) : fChain(0) , _skipIfSignal(false)
{
  ...
  ofile=new TFile(Form("Histo_ch2_%s.root",outAppendix),"RECREATE");
...
Int_t B0_ch2::GetEntry(Long64_t entry)
{
  // Read contents of entry.
  if (!fChain) return 0;
  return fChain->GetEntry(entry);
}
Long64_t B0_ch2::LoadTree(Long64_t entry)
{
...
void B0_ch2::Show(Long64_t entry)
{
