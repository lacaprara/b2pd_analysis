...
do
 ...
 output=$(grep -H ^Total ${logs} | awk '{sum+=$3; print sum}' |tail -1)
 
 input=$(grep -H ^Total ${logs} | awk '{sum+=$3; print sum}' |tail -1)
...
print "{0:10} ({1:4d}): IN: {2:0.4f}M OUT: {3:0.3f}M Eff: {4:.3}e-2 +/- {5:.2}e-2".format("$what.$channel",$nfiles,$input, $output/1.e6,eff*100.,sigmaEff*100.)
